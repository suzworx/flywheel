...
func runValidate(args []string) {
	...
	if len(res.GatesUnrun) > 0 {
		fmt.Printf("%s note: the worker never ran gate(s) %s itself; its report's claims about them are unmeasured\n", task, strings.Join(res.GatesUnrun, ", "))
	}
