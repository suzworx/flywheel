...
func init() {
	...
	registerHelp("run", "flywheel run <task> [--dir DIR] [--worker NAME] [--model MODEL] [--resume] [--force-model] [--delta FILE] [--allow-overlap] [--start-timeout DURATION] [--stall-timeout DURATION]", func() *flag.FlagSet { fs, _ := runFlags(); return fs })
}
...
func runUsage(w io.Writer) {
	fmt.Fprintln(w, "usage: flywheel run <task> [--dir DIR] [--worker NAME] [--model MODEL] [--resume] [--force-model] [--delta FILE] [--allow-overlap] [--start-timeout DURATION] [--stall-timeout DURATION]")
}
...
func runRun(args []string) {
	...
	res, err := flywheel.Run(o.dir, flywheel.RunOptions{
		...
		DeltaPath: o.delta, AllowOverlap: o.allowOverlap, StartTimeout: o.startTimeout, StallTimeout: o.stallTimeout,
