...
| `flywheel doctor [--worker NAME]` | available | Probe every configured model and classify its availability (exit 0/1); warns on stderr, without failing, when git tracks the ledger (`.flywheel/events.jsonl` or the shard directory `.flywheel/events/`); names the integration branch and its source (`integration.branch` or detected) and warns when a configured one does not resolve. |
