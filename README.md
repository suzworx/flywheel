...
| `flywheel rebase <task> [--onto REF] [--dir DIR]` | available ([#414](https://github.com/suzworx/flywheel/issues/414)) | A unit branched from another unit's `fw/<T>` whose base then landed as a squash shows `stacked` on the andon, and `flywheel land` refuses it (rule `stacked`); this rebases `fw/<task>` in its task worktree onto main and records the new base. Exit 0 rebased, 1 conflicts (aborted, paths listed) or error, 2 usage. |
| `flywheel recover [--json] [--apply] [--all] [--dormant-after DUR] [--dir DIR]` | available ([#422](https://github.com/suzworx/flywheel/issues/422)) | Integrity, every unit's world against the ledger, and one next action per unit with its command; read-only unless `--apply` (safe actions only, recorded as `recovered`). Rule failures on landed units are history and never fail integrity. Units with no event for `--dormant-after` (default `168h`) are dormant: summarised, never acted on by `--apply`. `--all` lists landed and dormant units in full. Exit 0 when integrity passes and nothing needs `investigate`, else 1. |
...
| `flywheel lint` | available | Check a brief for problems: owns, gate, goal, report contract, owns paths (exit 0/1); warns when no gate runs the full suite or owns misses a changed Go package's importer tests (`lint` config section). |
