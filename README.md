...
| `flywheel verify` | available | Check the event log against the transition rules T1, T3, T4, T5, T8, R1 (no inspected pass while a blocking review finding was open) and P1 (no inspected pass without a complete review panel, where it applies) (exit 0/6). `--log` also checks the event log's hash chain. |
...
| `flywheel review <task> --agent --panel --session <session> [--round N] [--fix [--rounds N] [--fix-worker NAME] [--worktree]]` | available ([#420](https://github.com/suzworx/flywheel/issues/420)) | Run the review panel: one persona per `review.panel` dimension, sequentially, each allowed findings only in its own dimension; with `--fix`, each loop round is a whole panel. Prints the findings, then the verdict matrix (`<dimension> pass\|correct\|missing`); exits 0 when every dimension is pass, 1 otherwise, 6 on a refusal. |
