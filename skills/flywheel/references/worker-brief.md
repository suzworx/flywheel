...
| blocked | rc 0, reason `stop`, and a `permission-denied` signal (the claude result line's `permission_denials`, named in the finished note; a Bash denial names the matched deny pattern and segment, or `unattributed`) | the harness denied a tool the unit needs: fix the path or the policy, then redispatch; never help it around the block. |
