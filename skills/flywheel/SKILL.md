...
- After publishing a release, run `flywheel audit --release <version> --session <auditor>` from a session that did not build it: it checks the tag, that the changelog cites exactly the merged feat/fix commits, that the published binary verifies and reports the version, that the changelog's `flywheel ...` spans name real commands and flags, and that README and the operator skill name every command. Never announce a release whose audit is not `pass` (exit 5 fail, 8 inconclusive).
