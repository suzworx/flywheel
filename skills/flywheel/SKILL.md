...
| Machine gauges | supervisor (the CLI, no model) | none: `flywheel supervise` (#55); `--resume-limited` also re-dispatches a rate-limited unit once its model's reset has passed (#472); `flywheel controller` does it automatically every tick (`controller.auto_resume`, #528), `--resume-limited` is the one-shot form |
