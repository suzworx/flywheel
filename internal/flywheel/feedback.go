...
// CheckLearningsOwned reports whether .flywheel/learnings.md may be
// overwritten: nil when the file does not exist or already carries the
// flywheel marker; otherwise an error naming the path and telling the
// operator to move it aside (or delete it) and re-run. feedback add and
// dismiss call it before appending an event, so a refusal records nothing.
func CheckLearningsOwned(dir string) error {
	...
	if !strings.Contains(string(b), learningsMarker) {
		return fmt.Errorf("%s exists without the flywheel marker and looks hand-maintained; move it aside (or delete it) and re-run — flywheel will not overwrite it", dot)
	}
	return nil
}
...
// <dir>/learnings.md is removed only when it carries the marker too (after the
// new file is written, so a failed write never orphans the old copy).
func WriteLearningsFile(dir string, views []LearningView) error {
	if err := CheckLearningsOwned(dir); err != nil {
		return err
	}
	...
	ob, err := os.ReadFile(old)
	if err == nil {
		if strings.Contains(string(ob), learningsMarker) {
			if err := os.Remove(old); err != nil {
				return fmt.Errorf("remove %s: %w", old, err)
			}
		}
	} else if !os.IsNotExist(err) {
		return fmt.Errorf("read %s: %w", old, err)
	}
	return nil
}
