...
- In a **sibling worktree** (another git worktree of the same repository, compared against the
  ...
  whose brief owns it and which has not landed (any status except `landed`) — its own ledger is the
  authority for its own worktree, and a unit that passed inspection still owns the uncommitted
  edits made in its worktree afterwards (issue #278). A path no not-yet-landed task there owns is
  still `outside`.
